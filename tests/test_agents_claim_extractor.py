...
def _mock_stream(text: str):
    ...
    mock_stream.get_final_message.return_value = _mock_response(text)
    return mock_stream


def test_clean_json_parsed_correctly():
    with patch("src.agents.claim_extractor._client") as mock_client:
        mock_client.messages.stream.return_value = _mock_stream(_minimal_claims_json())
...
def test_json_with_backtick_fences_parsed():
    ...
    with patch("src.agents.claim_extractor._client") as mock_client:
        mock_client.messages.stream.return_value = _mock_stream(wrapped)
        graph = extract_claims("paper text here")
...
def test_json_with_surrounding_prose_extracted():
    ...
    with patch("src.agents.claim_extractor._client") as mock_client:
        mock_client.messages.stream.return_value = _mock_stream(prose)
        graph = extract_claims("paper text here")
    assert len(graph.claims) == 1
...
def test_multi_claim_graph_returned():
    with patch("src.agents.claim_extractor._client") as mock_client:
        mock_client.messages.stream.return_value = _mock_stream(_two_claim_json())
        graph = extract_claims("paper text here")
...
def test_invalid_json_raises():
    with patch("src.agents.claim_extractor._client") as mock_client:
        mock_client.messages.stream.return_value = _mock_stream("not json at all")
